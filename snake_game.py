...
def gameloop():
    ...
    while not exit_game:
        if game_over:
            ...
            # window.fill(white)
            # screen_text("Game over! press Enter continue", red, 70, 300)
            # for i in range(0,1000,100):
            #     for j in range(0,1000,100):
            #         pygame.draw.rect(window, black, [i, j, 1, 1])
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:
                        open("highscore.txt", "r+").write(str(highscore))
                        welcome()
        else:
            for event in pygame.event.get():
            ...
            pygame.draw.rect(window, black, [s_x, s_y, snake_size, snake_size])
            pygame.draw.rect(window, red, [f_x, f_y, food_size, food_size])
            s_x += v_x
            s_y += v_y
